...
@section('title', 'Editar Sangria')

@section('content')
    <x-layouts.breadcrumb title="Editar Sangria" :breadcrumbs="[
    ...
    <div class="container bg-white rounded p-4 shadow-sm">
        ...
        @if ($caixa)
            <div class="alert d-flex align-items-center gap-3 mb-4"
                ...
                <div>
                    <div class="fw-semibold" style="color:#166534;">Espécie disponível (incluindo esta sangria)</div>
                    <div class="small text-muted">O valor não pode exceder esse montante.</div>
                </div>
        ...
        <form action="{{ route('sangrias.update', $sangria->id) }}" method="POST">
            @csrf
            @method('PATCH')
            ...
            <div class="row">
                <div class="col-md-4">
                    ...
                    <input type="date" name="data_retirada" id="data_retirada"
                           value="{{ old('data_retirada', $sangria->data_retirada->format('Y-m-d')) }}"
                           required
                ...
                <div class="col-md-4">
                    ...
                    <select name="categoria" id="categoria" required
                        ...
                        @foreach (\App\Models\Sangria::$categorias as $key => $label)
                            <option value="{{ $key }}"
                                {{ old('categoria', $sangria->categoria) === $key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" name="valor" id="valor"
                           value="{{ old('valor', number_format((float) $sangria->valor, 2, '.', '')) }}"
                           min="0.01" step="0.01"
                           required
                ...
                <div class="col-12">
                    ...
                    <input type="text" name="observacao" id="observacao"
                           value="{{ old('observacao', $sangria->observacao) }}"
