...
@section('content')
    <x-layouts.breadcrumb title="Nova Ocorrência" :breadcrumbs="[['name' => 'Ocorrências', 'route' => 'ocorrencias.index'], ['name' => 'Nova Ocorrência']]" />
            ...
            <div class="row">
                <div class="col-4">
                    ...
                    <select name="produto_id" id="produto_id" required
                        class="form-select p-3 @error('produto_id') is-invalid @enderror">
                        <option value="">Selecione...</option>
                        @foreach ($produtos as $produto)
                            <option value="{{ $produto->id }}" {{ old('produto_id') == $produto->id ? 'selected' : '' }}>
                                {{ $produto->name }} (Estoque:
                                {{ number_format((int) $produto->total_quantity, 0, ',', '.') }}) </option>
                        @endforeach
                    </select>
                    @error('produto_id')
                ...
                <div class="col-4">
                    ...
                    <input type="text" value="{{ auth()->user()->name }}" disabled class="form-control p-3 bg-light">
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <textarea name="motivo" id="motivo" rows="4" required
                        ...
                        placeholder="Ex: garrafa quebrada, produto sem gás, item estragado...">{{ old('motivo') }}</textarea>
                    @error('motivo')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
